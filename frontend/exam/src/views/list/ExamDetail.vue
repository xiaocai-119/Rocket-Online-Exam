  ...
  <a-layout>
    <a-layout-header class="header" style="color: #fff">
      ...
      <span style="float: right;">
        <span style="margin-right: 60px; font-size: 20px" v-if="examDetail.exam"> <count-down :on-end="FinishUpdateExam" :target="new Date(examRecordDeail.examJoinDate).getTime()+examDetail.exam.examTimeLimit*60000" /></span>
        <a-button type="danger" ghost style="margin-right: 60px;" @click="FinishUpdateExam()">交卷</a-button>
...
<script>
import { getExamDetail, getQuestionDetail, FinishUpdateExam, getExamRecordDetail } from '../../api/exam'
...
export default {
  ...
  mounted () {
    ...
    let examRecord = {}
    let exam = {}
    getExamRecordDetail(this.$route.params.id)
      .then(res => {
        if (res.code === 0) {
          console.log(new Date(res.data.examRecord.examJoinDate).getTime())
          // 赋值考试对象
          ...
          if (res.data.examRecord.examIsEnd) {
            ...
            examRecord = res.data.examRecord
  ...
  methods: {
    ...
    FinishUpdateExam () {
      ...
      FinishUpdateExam(this.$route.params.id, this._mapToJson(this.answersMap))
        .then(res => {
          if (res.code === 0) {
