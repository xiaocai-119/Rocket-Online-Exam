  ...
  <a-card :bordered="false">
    ...
    >
      ...
      <span slot="action" slot-scope="text, record">
        <template>
          <a @click="handleOutput(record)">导出</a>
        </template>
      </span>
...
import { getExamStudentExamdata, ExamdataOutput } from '../../api/exam'
...
export default {
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        ...
        return getExamStudentExamdata(this.queryParam)
          .then(res => {
            if (res.code === 0) {
              console.log(res)
              return res.data
  ...
  methods: {
    ...
    // 处理下导出的数据
    handleOutput (record) {
      ExamdataOutput(record).then(res => { // 处理返回的文件流
        const blob = new Blob([res])
        const fileName = record.exam.examName + '.xlsx'
